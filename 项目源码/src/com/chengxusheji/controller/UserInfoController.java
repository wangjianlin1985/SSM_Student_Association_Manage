...
public class UserInfoController extends BaseController {
	...
	public void list(String user_name,String name,String schoolName,String specialInfo,String birthday,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		List<UserInfo> userInfoList = userInfoService.queryUserInfo(user_name, name, schoolName, specialInfo, birthday, page);
	    /*计算总的页数和总的记录数*/
	    userInfoService.queryTotalPageAndRecordNumber(user_name, name, schoolName, specialInfo, birthday);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String user_name,String name,String schoolName,String specialInfo,String birthday,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		...
		if (specialInfo == null) specialInfo = "";
		if (birthday == null) birthday = "";
		List<UserInfo> userInfoList = userInfoService.queryUserInfo(user_name, name, schoolName, specialInfo, birthday, currentPage);
	    /*计算总的页数和总的记录数*/
	    userInfoService.queryTotalPageAndRecordNumber(user_name, name, schoolName, specialInfo, birthday);
	    /*获取到总的页码数目*/
	    int totalPage = userInfoService.getTotalPage();
	...
	public void OutToExcel(String user_name,String name,String schoolName,String specialInfo,String birthday, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(user_name == null) user_name = "";
        if(name == null) name = "";
        if(schoolName == null) schoolName = "";
        if(specialInfo == null) specialInfo = "";
        if(birthday == null) birthday = "";
        List<UserInfo> userInfoList = userInfoService.queryUserInfo(user_name,name,schoolName,specialInfo,birthday);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "UserInfo信息记录"; 
        String[] headers = { "用户名","姓名","性别","用户照片","学校学院","年级专业","民族","政治面貌","出生日期","联系电话"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<userInfoList.size();i++) {
        	UserInfo userInfo = userInfoList.get(i); 
        	dataset.add(new String[]{userInfo.getUser_name(),userInfo.getName(),userInfo.getSex(),userInfo.getUserPhoto(),userInfo.getSchoolName(),userInfo.getSpecialInfo(),userInfo.getNation(),userInfo.getPoliticalStatus(),userInfo.getBirthday(),userInfo.getTelephone()});
        }
        /*
