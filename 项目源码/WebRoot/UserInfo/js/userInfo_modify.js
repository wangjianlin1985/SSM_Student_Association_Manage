$(function () {
	...
	UE.delEditor('userInfo_introduce_edit');
	var userInfo_introduce_edit = UE.getEditor('userInfo_introduce_edit'); //个人介绍编辑器
	userInfo_introduce_edit.addListener("ready", function () {
		 // editor准备好之后才可以使用 
		 ajaxModifyQuery();
	}); 
	...
	$.ajax({
		...
		success : function (userInfo, response, status) {
			...
			if (userInfo) { 
				...
				$("#userInfo_schoolName_edit").validatebox({
					required : true,
					missingMessage : "请输入学校学院",
				});
				$("#userInfo_specialInfo_edit").val(userInfo.specialInfo);
				$("#userInfo_specialInfo_edit").validatebox({
					required : true,
				...
				userInfo_introduce_edit.setContent(userInfo.introduce);
			} else {
